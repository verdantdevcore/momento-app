    ...
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--bg-base)', display: 'flex', flexDirection: 'column' }}>
      <header style={{ backgroundColor: 'var(--bg-surface)', borderBottom: '1px solid var(--border)', padding: '0 1.5rem', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <Link href="/dashboard" style={{ height: '32px', paddingLeft: '0.875rem', paddingRight: '0.875rem', borderRadius: '0.5rem', border: '1px solid var(--border)', backgroundColor: 'var(--bg-input)', color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 600, display: 'flex', alignItems: 'center', textDecoration: 'none' }}>
            ← Dashboard
          </Link>
          <ThemeToggle />
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        <aside style={{ width: '200px', backgroundColor: 'var(--bg-surface)', borderRight: '1px solid var(--border)', padding: '1.25rem 0.75rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', flexShrink: 0 }}>
          ...
          {([['overview', '📊', 'Overview'], ['hosts', '👤', 'Hosts']] as const).map(([id, icon, label]) => (
            <button key={id} onClick={() => setActiveSection(id)} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', padding: '0.625rem 0.75rem', borderRadius: '0.625rem', color: activeSection === id ? 'var(--text-primary)' : 'var(--text-muted)', fontSize: '0.875rem', fontWeight: 600, background: activeSection === id ? 'rgba(85,107,47,0.15)' : 'none', border: activeSection === id ? '1px solid rgba(85,107,47,0.3)' : '1px solid transparent', cursor: 'pointer', textAlign: 'left', width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', animation: 'fadeIn 0.2s ease' }}>
                  ...
                  {statCards.map(card => (
                    <div key={card.label} style={{ backgroundColor: 'var(--bg-surface)', borderRadius: '0.875rem', border: '1px solid var(--border)', padding: '1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', fontWeight: 600, margin: 0, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{card.label}</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
                {[
                  ...
                  <div key={item.label} style={{ backgroundColor: 'var(--bg-surface)', borderRadius: '0.875rem', border: '1px solid var(--border)', padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                    ...
                    <p style={{ color: 'var(--text-primary)', fontSize: '1.75rem', fontWeight: 800, margin: 0, letterSpacing: '-0.02em' }}>{item.value}</p>
                  </div>
                ))}
              </div>
              ...
              {categories.length > 0 && (
                <div>
                  ...
                  {topCategory && (
                    <div style={{ backgroundColor: 'var(--accent)', borderRadius: '1rem', padding: '1.5rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', position: 'relative', overflow: 'hidden' }}>
                      ...
                      <div style={{ position: 'relative', zIndex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '0.5rem' }}>
                          <span style={{ backgroundColor: 'rgba(247,231,206,0.2)', color: '#F7E7CE', fontSize: '0.7rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: '999px', border: '1px solid rgba(247,231,206,0.3)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                            🏆 Top Category
                          </span>
                        </div>
                        <p style={{ color: '#F7E7CE', fontSize: '2rem', fontWeight: 800, margin: 0, letterSpacing: '-0.02em' }}>
                          {categoryEmojis[topCategory.category] ?? ''} {topCategory.category}
                        </p>
                        <p style={{ color: 'rgba(247,231,206,0.75)', fontSize: '0.875rem', margin: '0.375rem 0 0' }}>
                      ...
                      <div style={{ textAlign: 'right', position: 'relative', zIndex: 1, flexShrink: 0 }}>
                        <p style={{ color: '#F7E7CE', fontSize: '3.5rem', fontWeight: 900, margin: 0, lineHeight: 1, letterSpacing: '-0.04em' }}>{topCategory.count}</p>
                  ...
                  {restCategories.length > 0 && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem' }}>
                      {restCategories.map((cat, i) => (
                        <div key={cat.category} style={{ backgroundColor: 'var(--bg-surface)', borderRadius: '0.875rem', border: '1px solid var(--border)', padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <span style={{ fontSize: '1.5rem' }}>{categoryEmojis[cat.category] ?? '📌'}</span>
                            <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem', fontWeight: 600 }}>#{i + 2}</span>
                          </div>
                          <div>
                            <p style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: '0.925rem', margin: 0 }}>{cat.category}</p>
                            <p style={{ color: 'var(--text-muted)', fontSize: '0.775rem', margin: '0.25rem 0 0' }}>{cat.count} event{cat.count !== 1 ? 's' : ''}</p>
                          </div>
                  ...
                  {categories.length === 0 && (
                    <div style={{ backgroundColor: 'var(--bg-surface)', borderRadius: '0.875rem', border: '1px solid var(--border)', padding: '2rem', textAlign: 'center' }}>
                      <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', margin: 0 }}>No categorised events yet</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                  {[...hosts].sort((a, b) => b.upload_count - a.upload_count).slice(0, 5).map((host, i) => (
                    <div key={host.id} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem' }}>
                      <div style={{ width: '32px', height: '32px', borderRadius: '50%', backgroundColor: i === 0 ? 'var(--accent)' : 'var(--bg-input)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 800, color: i === 0 ? '#F7E7CE' : 'var(--text-muted)', flexShrink: 0 }}>
                        {i + 1}
                      </div>
                      ...
                      <span style={{ color: 'var(--text-primary)', fontSize: '0.925rem', fontWeight: 700, flexShrink: 0 }}>{host.upload_count} uploads</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeSection === 'hosts' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', animation: 'fadeIn 0.2s ease' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
                ...
                <input type="text" placeholder="Search name or email..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: '260px', backgroundColor: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: '0.625rem', padding: '0.5rem 0.875rem', color: 'var(--text-primary)', fontSize: '0.875rem', outline: 'none' }} />
              </div>
